...
function doGet(e) {
  ...
  if (path === "sensor/accel/latest") return handleAccelLatest(e);
...
function handleCheckin(e) {
  try {
    ...
    var presenceId =
      "PR-" + String(presenceSheet.getLastRow()).padStart(4, "0");
...
function handleAccelBatch(e) {
  try {
    ...
    for (var i = 0; i < samples.length; i++) {
      var s = samples[i];
      rows.push([deviceId, s.x, s.y, s.z, s.ts || "", batchTs, recordedAt]);
    }
...
function handleAccelLatest(e) {
  try {
    ...
    if (last <= 1) return sendError("No accel data found");

    var data = sheet.getRange(2, 1, last - 1, 7).getValues();

    // Filter berdasarkan device_id (kolom 0)
    var rows = data.filter(function (r) {
      return r[0] === deviceId;
    });
    if (rows.length === 0)
      return sendError("No accel data found for device " + deviceId);

    // Sort descending by sample_ts (kolom 4) → ambil terbaru
